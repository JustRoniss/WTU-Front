    ...
    const CreateEvents: React.FC = () => {
        ...
        const onFinish = (values: EventFormValues) => {
            ...
            const startDateTime = moment.parseZone(`${startDate.format('YYYY-MM-DD')} ${startTime.format('HH:mm:ss')}`, 'America/Sao_Paulo');

            const endDateTime = moment.parseZone(`${endDate.format('YYYY-MM-DD')} ${endTime.format('HH:mm:ss')}`, 'America/Sao_Paulo');

            moment.locale('pt-br');

            const selectedUnits: UnitDTO[] = units.filter(u => unit.includes(u.id.toString())).map(u => ({ id: u.id }));
                                ...
                                >
                                    <Select
                                        ...
                                        options={units.map(unit => ({
                                            label: unit.name,
                                            value: unit.id.toString() // Convert to string
                                        }))}
                                    />
                                </Form.Item>
                                <Form.Item
